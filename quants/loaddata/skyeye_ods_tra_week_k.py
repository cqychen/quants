...
def load_data():
    ...
    for tmp_stock_code in stock_code:
        tempnum = tempnum + 1
        start_date=get_date_add_days(get_max_date_stock_w(tmp_stock_code),1) #在最大天数加一天作为日期
        end_date=get_date_now()
        print(tempnum,tmp_stock_code,start_date,end_date)
        tmp_rs = ts.get_k_data(code=tmp_stock_code, start=start_date, end=end_date, ktype='W')
